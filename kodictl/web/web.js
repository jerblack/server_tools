...
class NowPlaying extends PartsBin {
    ...
    update(np) {
        this.#ui.title.textContent = np.title;
        this.#ui.date.textContent = np.date;
        this.#ui.id.textContent = np.id;
        this.#ui.playlist.textContent = np.playlist;
        BtnBox._this.playing = np.playing;
...
class HeaderMenu extends PartsBin {
    ...
    build() {
        ...
        const title = this._div("header-title", "kodictl");
    ...
    loadMenu(playlists) {
        for (const [k, v] of Object.entries(playlists)) {
            ...
            item.addEventListener("click", () => {
                core.sendCmd("start_playlist", { "name": k });
            });
...
class BtnBox extends PartsBin {
    ...
    build() {
        ...
        const r3 = document.createElement("row");
        const r4 = document.createElement("row");
        const r5 = document.createElement("row");
        this._add_to(r1, RcBlank.make(), RcBtn.make(_.up, "up"), RcBlank.make());
        this._add_to(r2, RcBtn.make(_.left, "left"), RcBtn.make("radio_button_checked", "select"), RcBtn.make(_.right, "right"));
        this._add_to(r3, RcBtn.make("arrow_back", "back"), RcBtn.make(_.down, "down"), RcBtn.make("clear", "delete"));
        this._add_to(r4, RcBtn.make("home", "home"), RcBtn.make("stop", "stop"), RcBtn.make("menu", "menu"));
        const pp = RcBtn.make(_.play, "play_pause");
        this._add_to(r5, RcBtn.make("skip_previous", "prev"), pp, RcBtn.make("skip_next", "next"));
        this._add_to_me(r1, r2, r3, r4, r5);
        this.#ui.playBtn = pp;
    }
