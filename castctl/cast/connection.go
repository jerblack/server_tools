...
func (c *Connection) connect(addr string, port int) error {
	...
	if err != nil {
		return errMsg(err, fmt.Sprintf("unable to connect to chromecast at '%s:%d'", addr, port))
	}
	c.connected = true
	return nil
}
...
func (c *Connection) Send(requestID int, payload Payload, sourceID, destinationID, namespace string) error {
	...
	if err != nil {
		return errMsg(err, "unable to marshal json payload")
	}
	...
	if err != nil {
		return errMsg(err, "unable to marshal proto payload")
	}
	...
	if err = binary.Write(c.conn, binary.BigEndian, uint32(len(data))); err != nil {
		return errMsg(err, "unable to write binary format")
	}
	if _, err = c.conn.Write(data); err != nil {
		return errMsg(err, "unable to send data")
	}
